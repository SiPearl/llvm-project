...
; CHECK:       [[VECTOR_BODY]]:
; CHECK-NEXT:    [[INDEX:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT:%.*]], %[[LOOP_LATCH7:.*]] ]
; CHECK-NEXT:    br label %[[INNER_LOOP1:.*]]
; CHECK:       [[INNER_LOOP1]]:
; CHECK-NEXT:    [[TMP7:%.*]] = phi i64 [ 0, %[[VECTOR_BODY]] ], [ [[TMP21:%.*]], %[[INNER_LOOP1]] ]
; CHECK-NEXT:    [[VEC_PHI:%.*]] = phi <4 x float> [ zeroinitializer, %[[VECTOR_BODY]] ], [ [[TMP9:%.*]], %[[INNER_LOOP1]] ]
; CHECK-NEXT:    [[VEC_PHI3:%.*]] = phi <4 x i1> [ splat (i1 true), %[[VECTOR_BODY]] ], [ [[TMP15:%.*]], %[[INNER_LOOP1]] ]
; CHECK-NEXT:    [[TMP20:%.*]] = getelementptr inbounds nuw float, ptr [[B]], i64 [[TMP7]]
; CHECK-NEXT:    [[TMP14:%.*]] = load float, ptr [[TMP20]], align 4
...
; CHECK-NEXT:    [[TMP9]] = select <4 x i1> [[VEC_PHI3]], <4 x float> [[TMP6]], <4 x float> [[VEC_PHI]]
; CHECK-NEXT:    [[TMP21]] = add nuw nsw i64 [[TMP7]], 1
...
; CHECK-NEXT:    br i1 [[DOTNOT]], label %[[LOOP_LATCH7]], label %[[INNER_LOOP1]]
; CHECK:       [[LOOP_LATCH7]]:
; CHECK-NEXT:    [[TMP17:%.*]] = getelementptr inbounds float, ptr [[A]], i64 [[INDEX]]
; CHECK-NEXT:    store <4 x float> [[TMP9]], ptr [[TMP17]], align 4
...
; CHECK-NEXT:    store float [[A_NEXT]], ptr [[A_ADDR]], align 4
; CHECK-NEXT:    [[I_NEXT]] = add nuw nsw i64 [[I]], 1
; CHECK-NEXT:    [[LOOP_EXITCOND:%.*]] = icmp eq i64 [[I_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[LOOP_EXITCOND]], label %[[EXIT]], label %[[LOOP_HEADER]], !llvm.loop [[LOOP3:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
entry:
...
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
; CHECK:       [[VECTOR_BODY]]:
; CHECK-NEXT:    [[N_VEC:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT1:%.*]], %[[LOOP_LATCH6:.*]] ]
; CHECK-NEXT:    [[VEC_IND:%.*]] = phi <4 x i64> [ <i64 0, i64 1, i64 2, i64 3>, %[[VECTOR_PH]] ], [ [[VEC_IND_NEXT:%.*]], %[[LOOP_LATCH6]] ]
; CHECK-NEXT:    br label %[[INNER_LOOP1:.*]]
; CHECK:       [[INNER_LOOP1]]:
; CHECK-NEXT:    [[VEC_PHI:%.*]] = phi <4 x i64> [ zeroinitializer, %[[VECTOR_BODY]] ], [ [[BROADCAST_SPLAT6:%.*]], %[[INNER_LOOP1]] ]
; CHECK-NEXT:    [[TMP25:%.*]] = phi i64 [ 0, %[[VECTOR_BODY]] ], [ [[INDEX_NEXT:%.*]], %[[INNER_LOOP1]] ]
; CHECK-NEXT:    [[VEC_PHI3:%.*]] = phi <4 x float> [ zeroinitializer, %[[VECTOR_BODY]] ], [ [[TMP44:%.*]], %[[INNER_LOOP1]] ]
; CHECK-NEXT:    [[TMP0:%.*]] = getelementptr inbounds nuw float, ptr [[B]], i64 [[TMP25]]
...
; CHECK-NEXT:    [[TMP15:%.*]] = load float, ptr [[TMP8]], align 4
; CHECK-NEXT:    [[TMP16:%.*]] = load float, ptr [[TMP10]], align 4
; CHECK-NEXT:    [[TMP17:%.*]] = load float, ptr [[TMP12]], align 4
; CHECK-NEXT:    [[TMP18:%.*]] = load float, ptr [[TMP14]], align 4
...
; CHECK-NEXT:    br i1 [[TMP51]], label %[[LOOP_LATCH6]], label %[[INNER_LOOP1]]
; CHECK:       [[LOOP_LATCH6]]:
; CHECK-NEXT:    [[TMP28:%.*]] = getelementptr inbounds float, ptr [[A]], i64 [[N_VEC]]
; CHECK-NEXT:    store <4 x float> [[TMP44]], ptr [[TMP28]], align 4
...
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
; CHECK:       [[VECTOR_BODY]]:
; CHECK-NEXT:    [[INDEX:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT:%.*]], %[[LOOP_LATCH_LOOPEXIT3:.*]] ]
...
; CHECK-NEXT:    br label %[[INNER_LOOP1:.*]]
; CHECK:       [[INNER_LOOP1]]:
; CHECK-NEXT:    [[TMP21:%.*]] = phi i64 [ 0, %[[VECTOR_BODY]] ], [ [[TMP22:%.*]], %[[INNER_LOOP_JOIN:.*]] ]
; CHECK-NEXT:    [[VEC_PHI2:%.*]] = phi <4 x i64> [ zeroinitializer, %[[VECTOR_BODY]] ], [ [[TMP20:%.*]], %[[INNER_LOOP_JOIN]] ]
; CHECK-NEXT:    [[TMP2:%.*]] = bitcast <4 x i1> [[TMP1]] to i4
; CHECK-NEXT:    [[DOTNOT:%.*]] = icmp eq i4 [[TMP2]], 0
; CHECK-NEXT:    br i1 [[DOTNOT]], label %[[INNER_LOOP_JOIN]], label %[[INNER_LOOP_ANYACTIVE:.*]]
; CHECK:       [[INNER_LOOP_ANYACTIVE]]:
; CHECK-NEXT:    [[A_ADDR:%.*]] = getelementptr inbounds nuw i64, ptr [[B]], i64 [[TMP21]]
; CHECK-NEXT:    [[A_LOAD:%.*]] = load i64, ptr [[A_ADDR]], align 8
; CHECK-NEXT:    [[TMP6:%.*]] = insertelement <4 x i64> poison, i64 [[A_LOAD]], i64 0
; CHECK-NEXT:    [[BROADCAST_SPLAT:%.*]] = shufflevector <4 x i64> [[TMP6]], <4 x i64> poison, <4 x i32> zeroinitializer
; CHECK-NEXT:    br label %[[INNER_LOOP_JOIN]]
; CHECK:       [[INNER_LOOP_JOIN]]:
; CHECK-NEXT:    [[TMP19:%.*]] = phi <4 x i64> [ [[BROADCAST_SPLAT]], %[[INNER_LOOP_ANYACTIVE]] ], [ poison, %[[INNER_LOOP1]] ]
...
; CHECK-NEXT:    br i1 [[TMP24]], label %[[LOOP_LATCH_LOOPEXIT3]], label %[[INNER_LOOP1]]
; CHECK:       [[LOOP_LATCH_LOOPEXIT3]]:
; CHECK-NEXT:    [[PREDPHI:%.*]] = select <4 x i1> [[TMP1]], <4 x i64> [[TMP20]], <4 x i64> [[WIDE_LOAD]]
; CHECK-NEXT:    store <4 x i64> [[PREDPHI]], ptr [[TMP0]], align 8
; CHECK-NEXT:    [[INDEX_NEXT]] = add nuw i64 [[INDEX]], 4
...
; CHECK-NEXT:    [[A_LOAD1:%.*]] = load i64, ptr [[A_ADDR1]], align 8
...
; CHECK-NEXT:    [[B_LOAD:%.*]] = load i64, ptr [[B_ADDR]], align 8
...
; CHECK-NEXT:    store i64 [[A_RES]], ptr [[A_ADDR1]], align 8
; CHECK-NEXT:    [[I_NEXT]] = add nuw nsw i64 [[I]], 1
; CHECK-NEXT:    [[LOOP_EXITCOND:%.*]] = icmp eq i64 [[I_NEXT]], [[N]]
...
; CHECK-NEXT:    [[BROADCAST_SPLATINSERT3:%.*]] = insertelement <4 x i64> poison, i64 [[L]], i64 0
; CHECK-NEXT:    [[BROADCAST_SPLAT2:%.*]] = shufflevector <4 x i64> [[BROADCAST_SPLATINSERT3]], <4 x i64> poison, <4 x i32> zeroinitializer
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
; CHECK:       [[VECTOR_BODY]]:
; CHECK-NEXT:    [[TMP28:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT:%.*]], %[[OUTER_LATCH_LOOPEXIT24:.*]] ]
; CHECK-NEXT:    [[VEC_IND:%.*]] = phi <4 x i64> [ <i64 0, i64 1, i64 2, i64 3>, %[[VECTOR_PH]] ], [ [[VEC_IND_NEXT:%.*]], %[[OUTER_LATCH_LOOPEXIT24]] ]
; CHECK-NEXT:    [[TMP2:%.*]] = mul <4 x i64> [[VEC_IND]], [[BROADCAST_SPLAT2]]
; CHECK-NEXT:    br label %[[MIDDLE_LOOP5:.*]]
; CHECK:       [[MIDDLE_LOOP5]]:
; CHECK-NEXT:    [[TMP60:%.*]] = phi i64 [ 0, %[[VECTOR_BODY]] ], [ [[TMP61:%.*]], %[[MIDDLE_LATCH_LOOPEXIT19:.*]] ]
; CHECK-NEXT:    [[VEC_PHI4:%.*]] = phi <4 x i1> [ [[TMP7]], %[[VECTOR_BODY]] ], [ [[TMP65:%.*]], %[[MIDDLE_LATCH_LOOPEXIT19]] ]
...
; CHECK-NEXT:    br label %[[INNER_LOOP7:.*]]
; CHECK:       [[INNER_LOOP7]]:
; CHECK-NEXT:    [[VEC_PHI6:%.*]] = phi <4 x i64> [ zeroinitializer, %[[MIDDLE_LOOP5]] ], [ [[BROADCAST_SPLAT29:%.*]], %[[PRED_STORE_CONTINUE14:.*]] ]
; CHECK-NEXT:    [[TMP71:%.*]] = phi i64 [ 0, %[[MIDDLE_LOOP5]] ], [ [[TMP54:%.*]], %[[PRED_STORE_CONTINUE14]] ]
; CHECK-NEXT:    [[VEC_PHI7:%.*]] = phi <4 x i1> [ [[TMP4]], %[[MIDDLE_LOOP5]] ], [ [[TMP58:%.*]], %[[PRED_STORE_CONTINUE14]] ]
...
; CHECK-NEXT:    [[TMP15:%.*]] = load float, ptr [[TMP1]], align 4
; CHECK-NEXT:    [[TMP16:%.*]] = fadd float [[TMP10]], [[TMP15]]
; CHECK-NEXT:    store float [[TMP16]], ptr [[TMP9]], align 4
...
; CHECK-NEXT:    store float [[TMP67]], ptr [[TMP21]], align 4
; CHECK-NEXT:    br label %[[PRED_STORE_CONTINUE10]]
; CHECK:       [[PRED_STORE_CONTINUE10]]:
; CHECK-NEXT:    [[TMP68:%.*]] = extractelement <4 x i1> [[VEC_PHI7]], i64 2
; CHECK-NEXT:    br i1 [[TMP68]], label %[[PRED_STORE_IF11:.*]], label %[[PRED_STORE_CONTINUE12:.*]]
...
; CHECK-NEXT:    [[TMP34:%.*]] = load float, ptr [[TMP70]], align 4
...
; CHECK-NEXT:    store float [[TMP40]], ptr [[TMP70]], align 4
; CHECK-NEXT:    br label %[[PRED_STORE_CONTINUE12]]
; CHECK:       [[PRED_STORE_CONTINUE12]]:
; CHECK-NEXT:    [[TMP41:%.*]] = extractelement <4 x i1> [[VEC_PHI7]], i64 3
; CHECK-NEXT:    br i1 [[TMP41]], label %[[PRED_STORE_IF13:.*]], label %[[PRED_STORE_CONTINUE14]]
...
; CHECK-NEXT:    [[TMP51:%.*]] = load float, ptr [[TMP50]], align 4
; CHECK-NEXT:    [[TMP52:%.*]] = fadd float [[TMP46]], [[TMP51]]
; CHECK-NEXT:    store float [[TMP52]], ptr [[TMP45]], align 4
...
; CHECK-NEXT:    br i1 [[DOTNOT]], label %[[MIDDLE_LATCH_LOOPEXIT19]], label %[[INNER_LOOP7]]
; CHECK:       [[MIDDLE_LATCH_LOOPEXIT19]]:
...
; CHECK-NEXT:    br i1 [[DOTNOT30]], label %[[OUTER_LATCH_LOOPEXIT24]], label %[[MIDDLE_LOOP5]]
; CHECK:       [[OUTER_LATCH_LOOPEXIT24]]:
...
; CHECK-NEXT:    [[A_LOAD:%.*]] = load float, ptr [[A_GEP]], align 4
; CHECK-NEXT:    [[B_LOAD:%.*]] = load float, ptr [[B_GEP]], align 4
; CHECK-NEXT:    [[RES:%.*]] = fadd float [[A_LOAD]], [[B_LOAD]]
; CHECK-NEXT:    store float [[RES]], ptr [[A_GEP]], align 4
...
; CHECK-NEXT:    br i1 [[OUTER_EXITCOND]], label %[[EXIT]], label %[[OUTER_LOOP]], !llvm.loop [[LOOP9:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
entry:
