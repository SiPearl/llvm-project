...
void IntrinsicLibrary::genAtomicCasCoarray(llvm::ArrayRef<fir::ExtendedValue> args) {
  ...
  mlir::Value stat = isStaticallyAbsent(args[4])
  ...
  fir::runtime::genAtomicCas(builder, loc, imageNum, handle, offset, old,
                             compare, value, stat);
}
...
void IntrinsicLibrary::genCoBroadcast(llvm::ArrayRef<fir::ExtendedValue> args) {
  ...
  mlir::Value refNone =
      builder
          .create<fir::AbsentOp>(loc, builder.getRefType(builder.getNoneType()))
          .getResult();
